...
fn resolve_link_order(obj: &ObjInfo) -> Result<Vec<ObjUnit>> {
    ...
    for (_section_index, section) in obj.sections.iter() {
        ...
        // absolute address.
        let init_end = section.address + section.data.len() as u64;
        let has_tail = (section.data.len() as u64) < section.size;
    ...
    for (section_index, section) in obj.sections.iter() {
        ...
        loop {
            ...
            if !split.common {
                ...
                let split_kind =
                    if data.is_empty() && section.kind != ObjSectionKind::Bss && end_off > phys_len
                    {
                        ObjSectionKind::Bss
                    } else {
                        section.kind
