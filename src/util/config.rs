    ...
    W: Write + ?Sized,
{
    ...
    for (name, start, end, unit) in &obj.pe_comment_sections {
        if unit.is_none() {
            writeln!(w, "\t{name:<11} type:comment vaddr:{start:#010X} end:{end:#010X}")?;
        }
    }
    for unit in obj.link_order.iter().filter(|unit| all || !unit.autogenerated) {
        ...
        while let Some((_section_index, section, addr, split)) = split_iter.next() {
            ...
            write!(
                ...
                split.rename.as_deref().unwrap_or(&section.name),
                addr + vaddr_offset,
                end + vaddr_offset
            )?;
        ...
        for (name, start, end, cunit) in &obj.pe_comment_sections {
            if cunit.as_deref() == Some(unit.name.as_str()) {
                writeln!(w, "\t{name:<11} type:comment start:{start:#010X} end:{end:#010X}")?;
            }
        }
    }
    Ok(())
...
struct SplitSection {
    ...
    skip: bool,
    /// `type:comment` — a [start, end) range of exestr comment bytes in the PE
    /// header padding, emitted as `.drectve` directives in this unit's object.
    comment: bool,
}
...
pub struct SectionDef {
    ...
    pub end: Option<u32>,
    /// `type:comment` — a [vaddr, end) range of exestr comment bytes in the PE
    /// header padding (outside any physical section). Read via
    /// [`read_splits_sections`] and re-emitted as `.drectve` directives; skipped
    /// by [`apply_splits`].
    pub comment: bool,
    /// `unit:<name>` — owning unit for a `type:comment` range; its `.drectve` is
    /// emitted into that unit's object (else a shared object).
...
fn parse_section_line(captures: Captures, state: &SplitState) -> Result<SplitLine> {
    if matches!(state, SplitState::Sections(_)) {
        ...
        for attr in captures["attrs"].split(' ').filter(|&s| !s.is_empty()) {
            if let Some((attr, value)) = attr.split_once(':') {
                match attr {
                    "type" if value == "comment" => section.comment = true,
                    "unit" => section.unit = Some(value.to_string()),
                    "type" => {
                        section.kind = Some(
    ...
    for result in r.lines() {
        ...
        match (&mut state, split_line) {
            ...
            (
                ...
                SplitLine::Section(SectionDef { name, kind, align, vaddr, end, comment, unit }),
            ) => {
                // Comment ranges live in the header padding, outside any physical
                // section; the COFF loader reads them. Retain the declaration so
                // it round-trips through a splits-file rewrite.
                if comment {
                    if let (Some(start), Some(stop)) = (vaddr, end) {
                        obj.pe_comment_sections.push((name, start, stop, unit));
                    }
                    continue;
                }
            ...
            (
                ...
                // it round-trips through a splits-file rewrite.
                if comment {
                    obj.pe_comment_sections.push((name, start, end, Some(unit.clone())));
                    continue;
                }
                ensure!(end >= start, "Invalid split range {:#X}..{:#X}", start, end);
                let (section_index, fallback_rename) = match obj.sections.by_name(&name)? {
                    Some(v) => Ok((v.0, None)),
                    None => {
                        if obj.kind == ObjKind::Executable {
                            // Section name is a COFF sub-section (e.g. .rdata$r) that was merged
                            // into a different PE section. Find by address range and record the
                            // intended COFF name as the rename so stubs emit the right section.
                            obj.sections
                                .with_range(start..end)
                                .map(|(idx, _)| (idx, Some(name.clone())))
                        } else {
                ...
                section.splits.push(
                    ...
                    ObjSplit {
                        ...
                        autogenerated: false,
                        skip,
                        rename: rename.or(fallback_rename),
                    },
                );
            }
            _ => {}
...
/// Read `type:comment` ranges from a splits file as `(start_va, end_va, unit)`.
/// A range in the `Sections:` block is shared (`unit = None` unless it carries a
/// `unit:` attribute); a range under a unit belongs to that unit.
pub fn read_comment_regions(path: &Utf8NativePath) -> Result<Vec<(u32, u32, Option<String>)>> {
    ...
    for result in file.lines() {
        let line = result?;
        match parse_split_line(&line, &state)? {
            SplitLine::SectionsStart => state = SplitState::Sections(0),
            SplitLine::Unit(unit) => state = SplitState::Unit(unit.name),
            SplitLine::Section(def) if def.comment => {
                let (Some(start), Some(end)) = (def.vaddr, def.end) else {
                    bail!("Comment section '{}' requires vaddr and end", def.name);
                };
                regions.push((start, end, def.unit));
            }
            SplitLine::UnitSection(sec) if sec.comment => {
                let unit = match &state {
                    SplitState::Unit(name) => name.clone(),
                    _ => bail!("Comment section '{}' outside a unit", sec.name),
                };
                regions.push((sec.start, sec.end, Some(unit)));
            }
            _ => {}
        }
    }
